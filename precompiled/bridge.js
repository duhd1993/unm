...
const insure=__webpack_require__(2290),select=__webpack_require__(2822),request=(__webpack_require__(743),__webpack_require__(5262)),{getManagedCacheStorage}=__webpack_require__(9220),format=song=>({id:song.hash,id_hq:song["320hash"],id_sq:song.sqhash,name:song.songname,duration:1e3*song.duration,album:{id:song.album_id,name:song.album_name}}),search=info=>{const url="http://mobilecdn.kugou.com/api/v3/search/song?keyword="+encodeURIComponent(info.keyword)+"&page=1&pagesize=10"
return request("GET",url).then((response=>response.json())).then((jsonBody=>{const list=jsonBody.data.info.map((song=>song.group.map(format))).flat(),matched=select(list,info)
return matched||Promise.reject()})).catch((()=>insure().kugou.search(info)))},track=song=>(song=>{const url="https://wwwapi.kugou.com/yy/index.php?r=play/getdata&hash="+song.id+"&dfid=2mSZvv2GejpK2VDsgh0K7U0O&mid=c18aeb062e34929c6e90e3af8f7e2512&platid=4&_=1653050047389&album_id="+song.album.id
return request("GET",url).then((response=>response.json())).then((jsonBody=>jsonBody.data.play_url||Promise.reject()))})(song).then((resp=>resp||Promise.reject())).catch((()=>insure().kugou.track(song))),cs=getManagedCacheStorage("provider/kugou")
